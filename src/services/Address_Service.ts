...
class AddressService {
  static async createAddress(AddressData: IAddresses) {
    try {
      ...
      throw new Error(`${path.resolve(__filename)} Unable to create Address`);
    }
  }
  ...
  static async updateAddress(
    ...
    try {
      ...
      const response = await Address.findByIdAndUpdate(idAddress, AddressData, {
        new: true,
      });
      return response;
    } catch (error) {
      console.log("errorS", error);
      throw new Error(`${path.resolve(__filename)} Unable to update Address`);
    }
  }
  ...
  static async getAllAddress(
    ...
    try {
      ...
      throw new Error(`${path.resolve(__filename)} Unable to get all Address`);
    }
  }
  ...
  static async deleteAddress(idAddress: mongoose.Types.ObjectId) {
    try {
      ...
      throw new Error(`${path.resolve(__filename)} Unable to delete Address`);
    }
  }
}
